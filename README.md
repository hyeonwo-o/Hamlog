...
- 상단의 브라우저 임시 저장 상태와 서버 저장 시각은 서로 다릅니다. 브라우저 복구본이 있으면 복구 또는 삭제를 선택하기 전 서버 저장을 막아 확인하지 않은 사본을 보호합니다.
...
검색 결과에는 본문 전체 대신 일치한 부분의 짧은 발췌를 표시합니다. 하위 카테고리 필터를 먼저
적용한 뒤 최신순 최대 25편을 반환하며, 비공개 글과 아직 공개되지 않은 예약 글은 제외합니다.

검색어(`q`)와 카테고리는 URL에 보존되어 글을 읽은 뒤 돌아오거나 새로고침해도 유지됩니다.
